...
-- TODO (Phase 4): implement KPI report SQL
...
churn_risk as (
    select
        ...
        count(*)                                 as customer_churn_risk_count
...
final as (
    select
        ...
        m.transaction_count,
        m.total_volume_nok,
        m.avg_transaction_nok,
        m.active_customer_count,
        m.fraud_rate_pct,
        m.anomaly_rate_pct,
        coalesce(cr.customer_churn_risk_count, 0) as customer_churn_risk_count,
        mac.norges_bank_rate,
        mac.cpi_index,
        current_timestamp()                       as _loaded_at
    from monthly_txns m
    left join macro     mac on m.report_month = mac.report_month
    left join churn_risk cr  on m.report_month = cr.report_month
)
